...
func (sc *serverConn) serve() {
	...
	if VerboseLogs {
		sc.vlogf("http2: server connection from %v on %p", sc.conn.RemoteAddr(), sc.srv)
	}
...
func (sc *serverConn) processHeaders(f *MetaHeadersFrame) error {
	...
	st.body = req.requestBody.pipe // may be nil
	st.declBodyBytes = req.length
...
func (sc *serverConn) newStream(id, pusherID uint32, state streamState) *stream {
	...
	if sc.srv.WriteTimeout != 0 {
		st.writeDeadline = time.AfterFunc(sc.srv.WriteTimeout, st.onWriteTimeout)
	}
...
func (sc *serverConn) newWriterAndRequest(st *stream, f *MetaHeadersFrame) (*responseWriter, *requestReader, error) {
	...
	if bodyOpen {
		if vv, ok := rp.header["Content-Length"]; ok {
			req.length, _ = strconv.ParseInt(vv[0], 10, 64)
		} else {
			req.length = -1
		}
		req.requestBody.pipe = &pipe{
			b: &dataBuffer{expected: req.length},
		}
	}
	return rw, req, nil
}
...
/*	for k := range opts.Header {
	...
	if err := checkValidHTTP2RequestProtocolHeaders(opts.Header); err != nil {
		return err
	}
	...
	msg := &startPushRequest{
		...
		method: opts.Method,
		url:    u,
		header: cloneProtocolHeader(opts.Header),
...
func (sc *serverConn) startPush(msg *startPushRequest) {
	...
	allocatePromisedID := func() (uint32, error) {
		...
		go sc.runHandler(rw, req, sc.handler)
...
func checkValidHTTP2RequestProtocolHeaders(h protocol.Header) error {
	for _, k := range connHeaders {
		if len(h.Get(k)) > 0 {
			return fmt.Errorf("request header %q is not valid in HTTP/2", k)
		}
